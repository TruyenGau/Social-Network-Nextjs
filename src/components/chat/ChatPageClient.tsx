...
const ChatPageClient: React.FC<ChatPageClientProps> = ({
    ...
    const [currentFriend, setCurrentFriend] =
        useState<IFollowItem["following"] | null>(null);
    ...
    const [selectedPending, setSelectedPending] = useState<IPendingRequest | null>(null);
    ...
    useEffect(() => {
        const fetchPendingRequests = async () => {
            try {
                ...
                const res = await fetch(
                    `${BACKEND_URL}/api/v1/chat/pending`,
                    {
                        headers: {
                            Authorization: `Bearer ${accessToken}`,
                        },
    ...
    useEffect(() => {
        ...
        const item = followings.find(
            (f) => f.following._id === initialUserId
        );
        ...
        const fetchGroups = async () => {
            try {
                ...
                const groupRooms: IRoom[] = roomsRaw
                    .filter(
                        (r: any) =>
                            r.type && r.type.toString().toLowerCase() === "group"
                    )
                    .map((r: any) => ({
                        _id: r._id,
                            ...
                            <>
                                <Avatar
                                    src={
                                        currentFriend.avatar
                                            ? `${BACKEND_URL}/images/${currentFriend.avatar}`
                                            : undefined
                                    }
                                ...
                                <Box>
                                    ...
                                    >
                                        ...
                                        {currentFriend.online ? "Đang hoạt động" : "Hoạt động 5p trước"}
                                    </Typography>
                                </Box>
                            </>
                        )}
                        ...
                        >
                            <Box
                                ...
                                sx={{
                                    ...
                                    fontWeight: 600,
                                    fontSize: 14,
                                    bgcolor: activeTab === "friends" ? "#3683d1ff" : "transparent",
                                ...
                                <List sx={{ py: 0 }}>
                                    {pendingRequests.map((req) => {
                                        ...
                                        const avatarUrl = sender.avatar
                                            ? `${BACKEND_URL}/images/${sender.avatar}`
                                            : undefined;
